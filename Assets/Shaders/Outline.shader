// NOTE: 实现了边缘光 rim light 
// 只在 ForwardBase 中加了 rim light，
// 所以如果场景中没有 directional light 会出问题

Shader "MyShaders/Rim Light Shader"{
	...
	[HDR] _EmissionColor("Emission Color", Color) = (0,0,0)
		...
		*/
		...
		fixed4 frag(v2f i) : SV_Target{
			...
			fixed3 specular = 0;// _LightColor0.rgb* _Specular.rgb* pow(saturate(dot(worldNormal, halfDir)), _Gloss);
			...
			half3 emission = tex2D(_EmissionMap, i.uv) * _EmissionColor;

			return fixed4(ambient + (diffuse + specular) * atten + rim + emission, 1.0);
		}
...
//			#pragma multi_compile_fwdadd
			...
			fixed4 frag(v2f i) : SV_Target{
				...
				return fixed4((diffuse + specular) * atten + emission, 1.0);
			}
